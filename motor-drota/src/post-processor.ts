/**
 * ops#1083 — Post-processor F3 (anti-infantilização) + F5 (persona consistency Kids).
 *
 * Salvaged fresh contra `main` atual a partir de motor#58 (closed PHANTOM-MERGEABLE).
 * Apenas a lógica dos filtros foi portada — o patch destrutivo do branch original
 * em `server.ts` (que removia silently o bloco `repetition_inquiry` de motor#110)
 * NÃO foi portado. Integração em server.ts é feita de forma aditiva.
 *
 * F3 — anti-infantilização (mode: 'warn' por default):
 *   Detecta padrões condescendentes para teen 13yo (diminutivos vazios,
 *   exclamações de elogio genérico). Warn mode loga warnings sem bloquear.
 *   Strict mode (futuro) bloqueia.
 *
 * F5 — persona consistency Kids (mode: 'strict' sempre):
 *   Detecta frases de assistente genérico que vazam a identidade IA / quebram
 *   a persona (ex: "Como posso te ajudar?", "Como IA, eu..."). Sempre bloqueia
 *   + retry via callback `regenerate()` até 2 vezes adicionais.
 *
 * Orquestrador `applyPostProcessors` aplica F3 → F5 com retry loop em F5.
 */
...
const F5_PATTERNS: { pattern: RegExp; label: string }[] = [
  ...
  { pattern: /eu\s+(tamb[eé]m\s+)?adoro\s*[!]/gi, label: "entusiasmo_artificial_adoro" },
...
/**
 * Orquestrador F3 + F5. Order: sanitize (caller) → F3 warn → F5 strict (+ retry 2x).
 *
 * - Ambos pass → retorna texto original com `passed=true`.
 * - F3 blocked (strict): retorna sem retry (F3 strict é decisão Jun futura).
 * - F5 blocked: chama `regenerate()` até 2x; se alguma versão passar nos dois, retorna ela.
 *   Se todas falham, retorna última tentativa com `blocked=true` (caller decide se aborta).
 */
export async function applyPostProcessors(
  ...
  if (!f5Result.blocked && !f3Result.blocked) {
    return {
      text,
      passed: f3Result.passed && f5Result.passed,
      blocked: false,
